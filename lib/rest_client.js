...
function updateEntries(worksheetInfo, entries, callback) {
    ...
    async.each(entries, function(entry, callback) {
        var payload = util._extend({
                body: api.updateEntryRequest(entry),
                entityId: entry._id
            },
            worksheetInfo
        );

        executeRequest('update_entry', payload, function(err, body) {
            cache.clear();
            handleResponse(null, callback, err, body);
        });
    }, callback);
...
function deleteEntries(worksheetInfo, entityIds, callback) {
    ...
    async.each(entityIds, function(entityId, callback) {
        var payload = util._extend({entityId: entityId}, worksheetInfo);
...
function createColumns(workSheetInfo, fieldNames, startIndex, callback) {
    ...
    async.each(fieldNames, function(fieldName, callback) {
        var payload = api.createFieldRequest(fieldName, ++startIndex);
