 ...
 * @returns {*}
 */
function mutateObject(object, descriptor) {

    if (!isMutatorDescriptor(descriptor)) {
        return descriptor && typeof descriptor === 'object' ? descriptor : object;
    }

    var setDescriptor = descriptor['$set'];

    if (setDescriptor) {
        Object.keys(setDescriptor).forEach(function(key) {
            object[key] = setDescriptor[key];
        });
    }

    var dateDescriptor = descriptor['$currentDate'];

    if (dateDescriptor) {
        var now = new Date();

        Object.keys(dateDescriptor).forEach(function(key) {
            if (dateDescriptor[key]) {
                object[key] = now;
            }
        });
    }
